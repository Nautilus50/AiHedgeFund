...
| Frontend screens | Built — minimal, unstyled. Command Centre (with a KPI overview and an Operations section: pending verifications, recent decisions, parse failures, live queue depth — all real grouped SQL counts plus BullMQ, org-scoped), Strategy Library (filterable by state/symbol/timeframe/parity, pushed down to real SQL), Campaign detail (summary, committee queue, and an append-only audit timeline read from `audit_events` — spec 15/CLAUDE.md 9.4), Strategy/Backtest-run/Verification detail, Backtest Lab (launch a LOCAL_RUNNER run against a picked dataset). Equity/drawdown charts on the backtest-run page (two linked single-axis charts, never one dual-axis chart — spec 15.18); raw evidence tables remain available underneath, collapsed |
...
| Forward-test paper engine | First vertical slice built — token-authenticated TradingView webhook ingestion, async signal processing into deterministic paper fills (one open position at a time, no pyramiding), real equity/drawdown/metrics reusing the existing `@arf-os/metrics` functions unchanged, a two-axis live health endpoint (infrastructure vs strategy performance, CLAUDE.md 16.3), a persisted health-snapshot history (operator/platform-scheduled `health-sweep` script, mirroring `reap-uploads`'s pattern rather than inventing an in-process scheduler), a drift report comparing forward metrics against a representative backtest run (reusing Validation Lab's `computeDegradation`, not a new statistical method — a 5-closed-trade floor guards against single-trade-dominated percentages), and a frontend (create form with a one-time token reveal, deployment detail page, health-history card, drift sub-page). See [ADR 0006](docs/adr/0006-forward-test-paper-engine.md) and [ADR 0012](docs/adr/0012-forward-drift-report-and-health-snapshots.md) — still no live market-data price feed, SSE still deliberately deferred |
